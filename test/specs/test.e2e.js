...
describe("Saucedemo Website Tests", () => {
  ...
  it("TC - [001] should login with valid credentials, hide password, and redirect to inventory page", async () => {
    async () => {
      expect(await loginPage.inputPassword.getAttribute("type")).toEqual(
        "password",
      );
  ...
  it("TC - [005] should add product to cart, logout, login again, and verify product in cart", async () => {
    ...
    const productName = await $(
      ".inventory_item:first-child .inventory_item_name",
    ).getText();
  ...
  it("TC - [006] should verify all sorting options for products", async () => {
    ...
    expect(productNamesElements.length).toBeGreaterThan(
      0,
      "No product names found for az sort",
    );
    let productNames = [];
  ...
  it("TC - [007] should open social media links in new tabs from the inventory page", async () => {
    ...
    const newWindowTwitter = allWindowsTwitter.find(
    ...
    const newWindowFacebook = allWindowsFacebook.find(
      (handle) => handle !== originalWindow,
    );
    ...
    const newWindowLinkedIn = allWindowsLinkedIn.find(
      (handle) => handle !== originalWindow,
    );
  ...
  it("TC - [008] should complete checkout process", async () => {
    ...
    const secondProductPrice = parseFloat(
      (
        await $(".inventory_item:nth-child(2) .inventory_item_price").getText()
      ).replace("$", ""),
    );
    ...
    expect(totalPrice).toEqual(
      parseFloat(
        (
          priceValues.reduce((sum, price) => sum + price, 0) + taxAmount
        ).toFixed(2),
      ),
    );
    ...
    expect(await checkoutPage.completeHeader.getText()).toEqual(
